...
- Changes:
  - Bỏ khung màu quanh logo (`bg-amber-400` instructor / `bg-primary` admin+dashboard) → logo sits trực tiếp trên nền sidebar, banner thành 1 surface liền mạch
  - Logo render **trắng "dương bản"** (`text-white`) trên nền dark navy (instructor) / slate-900 (admin) cho max contrast; dashboard vẫn dùng `text-primary` trên light surface
  - Instructor sidebar unify **`bg-blue-950`** cả Light + Dark mode (trước Light dùng `bg-blue-700` washed-out chữ trắng)
  - Brand label: `text-base` → `text-sm` + `truncate` + `min-w-0` → ellipsis khi hẹp thay vì wrap 2 dòng
  - `shrink-0` trên `<GvdLogo>` → logo giữ ratio 1:1 dù label dài
- `public/logo-gvd.svg` đã sẵn là "dương bản không nền" (white positive, không background) → không cần thay file, chỉ đổi Tailwind color class

### 🎨 Brand drop hyphen (23/04/2026 sáng) — v1.0.9

- **Feature**: Rename "GVD next-gen LMS" → "GVD next gen LMS" (bỏ gạch ngang). Commit `5b494a5`.
- Scope: **30 user-visible strings** — giống hệt rename v1.0.5 (Simvana → next-gen LMS):
  - Frontend: 14 files (pages, sidebars, manifest, hero, auth, certificate print)
  - Backend: 11 files (email templates, cert institution, Gemini prompt, Excel creator, SMTP from)
  - Database seed + `.env.production.example`
  - CONTEXT.md
- DB runtime: `SystemSetting.org.name` + `smtp.from` update qua SQL (existing rows không đổi theo seed default)
- Technical identifiers giữ nguyên: `@lms/*` packages, `lms-uploads` bucket, `LMS_*` env vars, container names, DB columns
...
- **Brand bỏ gạch ngang** (`5b494a5` / v1.0.9): "GVD next-gen LMS" → "GVD next gen LMS" — 30 user-visible strings. Code + DB đồng bộ. Không có logic change. Identical surface to v1.0.5 rename workflow.
- **Sidebar + logo polish** (`1a53cad` / v1.0.10): instructor preview phát hiện 3 issue — (1) khung vàng quanh logo tách banner thành 2 surface, (2) Light mode `bg-blue-700` washed-out chữ trắng, (3) label "GVD next gen LMS · Giảng viên" wrap 2 dòng ép logo méo. Fix: bỏ chip, unify `bg-blue-950` cả 2 mode, logo `text-white` dương-bản, label `text-sm + truncate + shrink-0` giữ ratio 1:1. Apply cho admin + instructor + dashboard sidebars.
